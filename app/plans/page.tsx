...
export const metadata: Metadata = {
  ...
  description: "Compare Auctrail Essential and Professional plans for surplus asset disposition workflows across public agencies, businesses, nonprofits, and other organizations, starting at $149 per month.",
  keywords: ["Auctrail pricing", "surplus asset disposition software pricing", "surplus sales management software", "business surplus software", "government surplus sales software"],
  alternates: { canonical: "https://auctrail.com/plans" },
  openGraph: { title: "Auctrail Plans and Pricing", description: "Two straightforward plans for organizations managing surplus asset disposition, starting at $149 per month.", url: "https://auctrail.com/plans", siteName: "Auctrail", type: "website" },
};
...
const plans = [
  {
    ...
    price: "149",
    annual: "1,788",
    description: "For smaller organizations and focused teams that need the complete Auctrail surplus-sales workflow without advanced automation or management customization.",
    capacity: [
      "Up to 5 active users",
      "Up to 5 departments or teams",
  ...
  },
  {
    ...
    price: "249",
    annual: "2,988",
    description: "For multi-department, multi-team, multi-location, or higher-volume operations that want Auctrail to do more of the management work through automation, advanced reporting, customization, and higher-touch service.",
    capacity: [
      "Up to 20 active users",
      "Unlimited departments or teams",
      "Advanced management dashboard and analytics",
      "Automated aging and workflow alerts",
      "Scheduled and advanced department or team reports",
      "Organization branding and workspace customization",
      "Custom organization domain option",
...
const coreCapabilities = [
  ...
  ["04", "Labels, reporting, and service", "Printable labels, QR access, search, reports, Excel and CSV exports, included storage for normal organizational use, secure hosting, updates, documentation, and support."],
];

const comparisonRows = [
  ["Active users included", "5", "20"],
  ["Departments or teams", "Up to 5", "Unlimited"],
  ["Surplus-sale records", "Unlimited", "Unlimited"],
  ["Storage for normal organizational use", "Included", "Included"],
  ...
  ["Department or team performance reporting", "Standard", "Advanced"],
  ["Workspace customization", "Standard Auctrail workspace", "Organization branding and customization"],
  ["Domain", "Auctrail-hosted domain", "Custom organization domain option"],
...
const frequentlyAskedQuestions = [
  ...
  ["Are surplus-sale records limited by plan?", "No. Essential and Professional both include unlimited surplus-sale records. Essential is limited to five active users and five departments or teams; Professional expands to twenty active users and unlimited departments or teams."],
  ...
  ["Is storage included?", "Yes. Both plans include storage for normal surplus-sale records, photos, and documents. Auctrail does not currently publish a fixed gigabyte allowance or automatic storage overage fee. Accounts with unusually high storage requirements may require a separate arrangement."],
  ["Can an organization change plans later?", "Yes. An Essential customer can move to Professional as its staff, departments, teams, reporting, automation, or customization needs grow without rebuilding its Auctrail surplus-sale records."],
  ...
  ["Can every GovDeals account use bulk upload immediately?", "Not necessarily. GovDeals Bulk Asset Upload is an account-enabled capability. The organization’s authorized GovDeals account must have it enabled before using an Auctrail GovDeals-compatible export for bulk upload."],
  ["Do annual prices include a discount?", "The annual totals shown are twelve months at the published monthly price. A separate annual-payment discount is not currently applied."],
  ["What if an organization needs more than 20 users?", "Contact Auctrail before subscribing. Larger deployments can be evaluated individually."],
];
...
function SiteFooter() { return <footer className="marketing-footer"><div className="marketing-shell footer-grid"><div className="footer-intro"><img src="/auctrail-logo-approved.jpg" alt="Auctrail" width="425" height="115" /><p>Surplus asset disposition software for public agencies, businesses, nonprofits, and other organizations preparing property for sale, tracking activity, documenting payment and pickup, and preserving completed-sale history.</p></div><div><strong>Platform</strong><Link href="/preview-progress/#platform">Capabilities</Link><Link href="/preview-progress/#workflow">Workflow</Link><Link href="/plans">Plans</Link><Link href="/preview-progress/#integrations">Integrations</Link></div><div><strong>Resources</strong><Link href="/faq">Frequently Asked Questions</Link><Link href="/support">Support</Link><a href="https://app.auctrail.com/login">Client login</a></div></div><div className="marketing-shell footer-bottom"><span>© {new Date().getFullYear()} Auctrail. All rights reserved.</span><span>Surplus Asset Disposition Software</span></div></footer>; }
...
export default function PlansPage() {
  ...
  return <main className={`marketing-site ${styles.plansSite}`}><script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(pricingJsonLd) }} /><MarketingHeader />
    ...
    <section className={styles.includedSection}><div className="marketing-shell"><div className={styles.centeredHeading}><p className="section-kicker">Included in both plans</p><h2>The complete surplus-sale record remains intact at either price.</h2><p>Auctrail does not remove core surplus intake, documentation, auction preparation, sale closeout, reporting, GovDeals-compatible export, or normal-use storage from Essential.</p></div><div className={styles.capabilityGrid}>{coreCapabilities.map(([number,title,description]) => <article key={number}><span>{number}</span><h3>{title}</h3><p>{description}</p></article>)}</div><div className={styles.govDealsNote}><CheckIcon /><div><strong>GovDeals bulk-upload prerequisite</strong><p>GovDeals Bulk Asset Upload must be enabled on the organization’s authorized GovDeals account before an Auctrail GovDeals-compatible export can be used for bulk upload.</p></div></div></div></section>
    <section className={styles.faqSection}><div className="marketing-shell"><div className={styles.faqHeading}><p className="section-kicker">Plan questions</p><h2>What organizations need to know before selecting a plan.</h2></div><div className={styles.faqGrid}>{frequentlyAskedQuestions.map(([question,answer]) => <article key={question}><h3>{question}</h3><p>{answer}</p></article>)}</div></div></section>
    <section className="final-cta"><div className="marketing-shell final-cta-inner"><div><p className="section-kicker light">A plan matched to organizational needs</p><h2>Choose the core platform or add management automation.</h2><p>Start with the way your organization operates today, then decide whether advanced automation, reporting, customization, and assisted service justify Professional.</p></div><div className="final-cta-actions"><Link className="button button-light" href="/demo">Request Demo <ArrowIcon /></Link><a className="button button-dark-outline" href="https://app.auctrail.com/login">Client login</a></div></div></section><SiteFooter /></main>;
}
